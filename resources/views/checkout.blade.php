        ...
        <div class="col-lg-8 mb-4">
            ...
            <div class="card mb-4" style="border: 2px solid #f0f0f0;">
                ...
                <div class="card-body">
                    ...
                    <!-- Address field now hidden by default, only shows when Delivery is selected -->
                    <div class="mb-3" id="addressField" style="display: none; transition: all 0.3s ease;">
                        <label class="form-label" style="font-weight: 600;">Alamat Pengiriman <span style="color: red;">*</span></label>
                        <textarea class="form-control" id="alamat" rows="3" placeholder="Masukkan alamat lengkap pengiriman" style="border-radius: 8px; padding: 0.75rem;"></textarea>
...
<script>
    ...
    function selectPayment(method) {
        if (!event || !event.currentTarget) {
            const paymentOption = document.getElementById(method);
            if (paymentOption) {
    ...
    function submitOrder() {
        ...
        if (metodePickup.value === 'delivery') {
    ...
    function processPayment() {
        ...
        if (!metodePickup || !metodePayment) {
            showToast('Silakan pilih metode pengambilan dan pembayaran', 'error');
            return;
        }

        const cart = JSON.parse(localStorage.getItem('cart') || '[]');
        const ongkir = metodePickup.value === 'delivery' ? 10000 : 0;
        ...
        fetch('{{ route("order.store") }}', {
            ...
            body: JSON.stringify({
                ...
                alamat: alamat,
                catatan: catatan,
                metode_pengambilan: metodePickup.value,
                metode_pembayaran: metodePayment.value,
                ongkir: ongkir,
                items: items
            })
        })
